package com.example.ezcalculator.common
...
fun CalculatorTabBar(navController: NavController) {
    Row(
        ...
        TabButton(title = "Calculator") {
            if (navController.currentBackStackEntry?.destination?.route != Screens.Calculation.rout) {
                navController.navigate(Screens.Calculation.rout)
            }
        }
        TabButton(title = "Exchange") {
            if (navController.currentBackStackEntry?.destination?.route != Screens.Exchange.rout) {
                navController.navigate(Screens.Exchange.rout)
            }
        }
    }
}
